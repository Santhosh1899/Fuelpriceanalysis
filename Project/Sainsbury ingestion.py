...
df_sainsbury = df_sainsbury.select(explode("stations").alias("stations")
).select("stations.*")

df_sainsbury = df_sainsbury.select(
    col("site_id"),
    col("brand"),
    col("address"),
    col("postcode"),
...
df_sainsbury=df_sainsbury.withColumn('Dateofoccurance',current_date())
df_sainsbury.dropDuplicates(subset=['address','postcode'])
max_date_query=spark.sql("SELECT MAX(Dateofoccurance) AS max_date FROM Fuelprice.Sainsbury").collect()[0]
max_date = max_date_query["max_date"]

if max_date!=date.today():
    df_sainsbury.write.mode("append").saveAsTable("Fuelprice.Sainsbury")

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS Fuelprice.Sainsbury (
